...
class MoeGatingGemmTorchOp(MoeGatingGemmOp):
    ...
    matching pre-rebase INTEL backend behaviour.

    Also fixes upstream bug where dst_dtype reads the wrong key ("dtype" instead
    of "dst_dtype") from args_dict.
    """

    def vendor_parser(self):
        # Fix: upstream MoeGatingGemmOp.prepare() sets self.dst_dtype from
        # args_dict["dtype"] instead of args_dict["dst_dtype"].
...
try:
    ...
    OP_MAPPING = {"moe_gating_gemm": MoeGatingGemmIpexOp}
except Exception:
    OP_MAPPING = {}
