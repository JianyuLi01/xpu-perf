...
from backends.INTEL.ops.store_kv_cache import generate_decode_data, generate_prefill_data, generate_prefill_session_cache_data
...
try:
    ...
    OP_MAPPING["rotary_embedding"] = RotaryEmbeddingIpexOp
except Exception:
    pass


class RotaryEmbeddingTorchWrapper(RotaryEmbeddingOp):
    """Wrapper that maps INTEL workload keys (q_seq_len, mode) to core-expected keys (q_len, attn_mode)."""
    def __init__(self, args_dict, backend, *args, **kwargs):
...
OP_MAPPING["rotary_embedding_torch"] = RotaryEmbeddingTorchWrapper
