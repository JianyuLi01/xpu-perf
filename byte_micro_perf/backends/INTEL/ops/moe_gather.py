...
class MoeGatherTorchOp(MoeGatherOp):
    def moe_gather_run(self, tensor_mapping):
        scatter_tokens = tensor_mapping["scatter_tokens"]
        scatter_tokens_offset = tensor_mapping["scatter_tokens_offset"]
        convergent_tokens = tensor_mapping["convergent_tokens"]

        # scatter_tokens_offset is 1D [real_scatter_tokens], each value is original token index
        # scatter_tokens is [real_scatter_tokens, hidden_size]
        # Accumulate scatter_tokens into convergent_tokens by original token index
        convergent_tokens.zero_()
        idx = scatter_tokens_offset.to(torch.int64).unsqueeze(1).expand_as(scatter_tokens)
        convergent_tokens.scatter_add_(0, idx, scatter_tokens)
...
try:
    ...
    OP_MAPPING["moe_gather"] = MoeGatherOp
except Exception:
    pass
OP_MAPPING["moe_gather_torch"] = MoeGatherTorchOp
